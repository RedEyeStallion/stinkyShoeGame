	...
	while 1:
		...
		if (multRect.x + 130 > mouse[0] > multRect.x and multRect.y + 130 > mouse[1] > multRect.y) and score >= upgradeCost:
			if click[0] == 1:
				...
				click = pygame.mouse.get_pressed()
				# Debugger: print("Click value: ", clickValue)
...
def getChance():
	...
	# if random number is >= 50, trigger mudslide event
		if(randNum*chance <= 50):
			mudslide()
	#if random number is >= 51, trigger stinky event
		if(randNum*chance >= 51):
			stinky()
...
def happyMeter():
	...
	if(hpoints <= 1):
		...
		mood = pygame.load.image("mood7.png")
		chance = 10
		return
	if(hpoints == 2):
		#load upset image
		mood = pygame.load.image("mood6.png")
		chance = 8
		return
	if(hpoints == 3):
		#load meh image
		mood = pygame.load.image("mood5.png")	
		chance = 6
		return
	if(hpoints == 4):
		#load neutral image
		mood = pygame.load.image("mood4.png")
		chance = 1.5
		return
	if(hpoints == 5):
		#load neutral_1 image
		mood = pygame.load.image("mood3.png")		
		chance = 1
		return
	if(hpoints == 6):
		#load happy image
		mood = pygame.load.image("mood2.png")
		chance = 0.5
		return
	if(hpoints >= 7):
		#load veryhappy image
		mood = pygame.load.image("mood1.png")
		chance = -1
		return
	...
	while 1:
		...
		if shoeRect.x + 600 > mouse[0] > shoeRect.x and shoeRect.y + 510 > mouse[1] > shoeRect.y:
			if click[0] == 1:
				...
				# reset left click[0] to 0
				click = pygame.mouse.get_pressed()
				#print("Click value now! ", clickValue)
